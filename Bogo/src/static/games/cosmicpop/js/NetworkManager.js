...
var whatServerNet = 'run_server';

if(_href.indexOf('localhost')>-1 || _href.indexOf('192.168')>-1){//개발서버,데브서버,DEV서버판별
    this.networkState = NET_STATE.LOCALHOST;
    whatServerNet = 'local_host';
    console.log('localhost');
}
else if(_href.indexOf('taeeset')>-1){
    this.networkState = NET_STATE.RUN_SERVER;
    whatServerNet = 'test_server';
    console.log('TEST_SERVER');
}
else if(_href.indexOf('dessav')>-1){
    this.networkState = NET_STATE.RUN_SERVER;
    whatServerNet = 'dev_server';
    console.log('DEV_SERVER');
}
else{
    this.networkState = NET_STATE.RUN_SERVER;
    whatServerNet = 'run_server';
    console.log('RUN_SERVER');
}

var nationServerNet = "korea_server";
if(_href.indexOf('game.com')>-1){
    nationServerNet = "korea_server";
    console.log('korea_server');
}
else if(_href.indexOf('www.game.jp')>-1){
    nationServerNet = "japan_server";
    console.log('japan_server');
}
else if(_href.indexOf('.yahoo')>-1){
    nationServerNet = "yahoo_server";
    console.log('yahoo_server');
}
else if(_href.indexOf('naver')>-1){
...
function NetworkManager(){
    ...
    if(link.indexOf('localhost')>-1){
        ...
        this.networkState = NET_STATE.TEST_SERVER;
    ...
    NetworkManager.prototype.GetShoppingListComplete = function (_data) {
        ...
        if(_data === undefined || _data == null){
            ...
            //         for(var i=0; i<4; i++){
            //             sHeartShop.itemList["greap"][""+i]["id"] = 7;
            ...
            //         for(var i=0; i<4; i++) {
            //             sHeartShop.itemList["yahoo"][""+i]["id"] = 7;
            ...
            //         //게스트 모드
            //         for (var i = 0; i < 4; i++) {
            //             sHeartShop.itemList["greapguest"][""+i]["id"] = 7;
