...
// 클라에서 누적 시킨 뒤 서버에 저장
var CASH_DATA_NAME = 'iCash'; // kook

// kData 안에 랭킹점수 항목의 이름으로
// 클라에서 누적 시킨 뒤 서버에 저장
var RANKVAL_DATA_NAME = null; // kook

// kData 내부에 하트관련된 항목의 이름으로.
var HEART_COUNT = 'iLife'; // kook
var HEART_TIME = 'fLifeTime'; // kook

// kData 내부의 Greap 포인트 항목
var GREAP_POINT = 'iMoviPoint'; // kook
 ...
 */
var sTopContainer = new PIXI.Container(); // kook
    ...
    */
    ...
    NetworkManager.prototype.SaveDataComplete = function (_data) {
        ...
        if(networkManager.networkState != NET_STATE.LOCALHOST){
            ...
            //  clientData[TIME_STAMP] = _data.data[TIME_STAMP]; // kook
                SaveDataInClient();
    ...
    NetworkManager.prototype.LoadDataComplete = function (_data) {
        ...
        if(loginTF == 1){
            if(_data['save'] != null)
            {
                ...
                // 클라 데이터 분리하지 않았을 경우 주석 처리
                //kData.calcedTimeStamp = _data['timeSTAMP'] - clientData[TIME_STAMP]; // kook
        ...
        if(cb_loadCompleteFunc !== undefined && cb_loadCompleteFunc != null){
            cb_loadCompleteFunc(_data); // yahooIN : 공통UI때문에 추가.
            cb_loadCompleteFunc = null;
        }
    ...
    NetworkManager.prototype.UseHeart = function (_useCount,_cb_func) {
        ...
        if(networkManager.networkState != NET_STATE.RUN_SERVER){
            if(kData[HEART_COUNT] == iLifeMax) // iHeartChargeMax 는 따로 지정 // kook
                kData[HEART_TIME] = fLifeAddTime; // fHeartChargeTime 는 따로 지정 // kook
