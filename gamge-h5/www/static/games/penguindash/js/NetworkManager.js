...
// 클라에서 누적 시킨 뒤 서버에 저장
var CASH_DATA_NAME = 'iGemCount';

// kData 안에 랭킹점수 항목의 이름으로
// 클라에서 누적 시킨 뒤 서버에 저장
var RANKVAL_DATA_NAME = "iRankScore";
...
var sTopContainer = new PIXI.Container();
// 서버 통신 중 버튼 못누르게 하는 pixi.js Graphic
var graphic_netWaiting = new PIXI.Graphics();
sNetworkLoading.addChild(graphic_netWaiting);
graphic_netWaiting.beginFill(0xff0000, 0);
graphic_netWaiting.drawRect(0, 0, 720, 1280);
graphic_netWaiting.endFill();
graphic_netWaiting.interactive = true;

// GameVIewSetting 에서 팝업UI 담아주면 될 거 같습니다.
// 혹은 payment애 있는 sGooglePlayJumpPanel = visible = true; 를 원하시는 코드로 변경해주시기 바랍니다.
var sGooglePlayJumpPanel = new PIXI.Container();
...
function NetworkManager(){
    ...
    // }else if(link.indexOf('.movigame.com') != -1){
    ...
    NetworkManager.prototype.SaveDataComplete = function (_data) {
        ...
        if(networkManager.networkState != NET_STATE.LOCALHOST){
            if(_data != null && _data.rank != null){
                ...
                if(bPayRank){
                    bPayRank = false;
                    networkManager.LoadRanking();
                } else {
                    networkManager.LoadRanking(function(){
                        if(!GAME.bRenewHighScore) GAME.ui_ranking.cb_setRankingPop();
                    });
                }
            }
            ...
            if(_data !== undefined && _data != null){
                ...
                GAME.curServerTime = _data.data[TIME_STAMP];
                SaveDataInClient();
    ...
    NetworkManager.prototype.LoadDataComplete = function (_data) {
        ...
        if(loginTF == 1){
            if(_data['save'] != null)
            {
                ...
                GAME.curServerTime = _data['timeSTAMP'];
        ...
        if(cb_loadCompleteFunc !== undefined && cb_loadCompleteFunc != null){
            cb_loadCompleteFunc(_data);//common
            cb_loadCompleteFunc = null;
        }
    ...
    NetworkManager.prototype.GetServerTime = function (cb_func) {
        ...
        if(networkManager.networkState != NET_STATE.LOCALHOST){
            console.log('networkManager.networkState != NET_STATE.LOCALHOST');
            console.log(networkManager.networkState);
            console.log(NET_STATE.LOCALHOST);
            getTimestamp();//获取服务器时间完成
        }else{
